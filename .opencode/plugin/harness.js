...
export const HarnessPlugin = async ({ client, directory }) => {
  ...
  // Derive harness skills dir from plugin location (works for both symlinked and local installs)
  const harnessSkillsDir = path.resolve(__dirname, '../../skills');
  const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

  // Helper to generate bootstrap content
  const getBootstrapContent = (compact = false) => {
    const usingHarnessPath = skillsCore.resolveSkillPath('using-harness', harnessSkillsDir, personalSkillsDir);
    if (!usingHarnessPath) return null;
...
**Skills naming (priority order):** project: > personal > harness:`
...
- Harness skills: \`harness:skill-name\`
- Project skills override personal, which override harness when names match`;

    return `<EXTREMELY_IMPORTANT>
You have harness.

**IMPORTANT: The using-harness skill content is included below. It is ALREADY LOADED - you are currently following it. Do NOT use the use_skill tool to load "using-harness" - that would be redundant. Use use_skill only for OTHER skills.**
  ...
  return {
    tool: {
      use_skill: tool({
        ...
        args: {
          skill_name: tool.schema.string().describe('Name of the skill to load (e.g., "harness:brainstorming", "my-custom-skill", or "project:my-skill")')
        },
        execute: async (args, context) => {
          const { skill_name } = args;

          // Resolve with priority: project > personal > harness
          ...
          if (forceProject || !skill_name.startsWith('harness:')) {
          ...
          // Fall back to personal/harness resolution
          if (!resolved && !forceProject) {
            resolved = skillsCore.resolveSkillPath(skill_name, harnessSkillsDir, personalSkillsDir);
          }
...
# ============================================`;
      ...
      find_skills: tool({
        description: 'List all available skills in the project, personal, and harness skill libraries.',
        args: {},
        execute: async (args, context) => {
          ...
          const harnessSkills = skillsCore.findSkillsInDir(harnessSkillsDir, 'harness', 3);

          // Priority: project > personal > harness
          const allSkills = [...projectSkills, ...personalSkills, ...harnessSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install harness skills to ~/.config/opencode/harness/skills/ or add project skills to .opencode/skills/';
          }
