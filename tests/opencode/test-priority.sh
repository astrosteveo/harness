...
# Verifies that skills are resolved with correct priority: project > personal > harness
...
# 1. Create in harness location (lowest priority)
mkdir -p "$HOME/.config/opencode/harness/skills/priority-test"
cat > "$HOME/.config/opencode/harness/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Harness version of priority test skill
---
# Priority Test Skill (Harness Version)
...
if [ -f "$HOME/.config/opencode/harness/skills/priority-test/SKILL.md" ]; then
    echo "  [PASS] Harness version exists"
else
    echo "  [FAIL] Harness version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides harness
echo ""
echo "Test 2: Testing personal > harness priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides harness)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_SUPERPOWERS_VERSION"; then
    echo "  [FAIL] Harness version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|harness" | head -10
fi

# Test 3: Test that project overrides both personal and harness
echo ""
echo "Test 3: Testing project > personal > harness priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Harness version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit harness: prefix bypasses priority
echo ""
echo "Test 4: Testing harness: prefix forces harness version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load harness:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_SUPERPOWERS_VERSION"; then
    echo "  [PASS] harness: prefix correctly forces harness version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] harness: prefix did not force harness version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
