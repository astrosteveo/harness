...
- Reference: `.codex/harness-codex` (lines 40-74)
...
- Reference: `.codex/harness-codex` (lines 97-136)
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/harness-codex` (lines 212-280)
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/harness-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/harness-codex` (add import at top)
...
Run: `node -c .codex/harness-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/harness-codex
...
**Files:**
- Modify: `.codex/harness-codex` (lines 40-74)
...
Run: `.codex/harness-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/harness-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/harness-codex` (lines 16-38, approximately)
...
Run: `.codex/harness-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/harness.js`
...
const harnessSkillsDir = path.join(homeDir, '.config/opencode/harness/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const HarnessPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/harness.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/harness.js
...
**Files:**
- Modify: `.opencode/plugin/harness.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "harness:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > harness)
...
Run: `node -c .opencode/plugin/harness.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the harness and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const harnessSkills = skillsCore.findSkillsInDir(
            harnessSkillsDir,
            'harness',
            3
          );
          ...
          const allSkills = [...personalSkills, ...harnessSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install harness skills to ~/.config/opencode/harness/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'harness:';
...
```javascript
    'session.started': async () => {
      ...
      const usingHarnessPath = skillsCore.resolveSkillPath(
        'using-superpowers',
        harnessSkillsDir,
        personalSkillsDir
      );

      let usingHarnessContent = '';
      if (usingHarnessPath) {
        const fullContent = fs.readFileSync(usingHarnessPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingHarnessContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Harness skills: \`harness:skill-name\` (from ~/.config/opencode/harness/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/harness')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/harness && git pull` to update superpowers.' :
        '';
...
**Below is the full content of your 'harness:using-superpowers' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**
...
### 1. Install Harness Skills

```bash
# Clone harness skills to OpenCode config directory
...
The plugin is included in the harness repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/harness/.opencode/plugin/harness.js`
...
ln -s ~/.config/opencode/harness/.opencode/plugin/harness.js .opencode/plugin/harness.js
```
...
Personal skills override harness skills with the same name.
...
1. Verify skills directory exists: `ls ~/.config/opencode/harness/skills`
...
- Documentation: https://github.com/astrosteveo/harness
```
...
Harness works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/harness-codex use-skill harness:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/harness-codex bootstrap | head -30`
...
ls -l .opencode/plugin/harness.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/harness.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/harness-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/harness-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/harness.js` created with tools and hooks
