...
The underlying skills (`harness:brainstorming`, `harness:executing-plans`, `harness:writing-plans`) remain available for Claude to invoke autonomously. This change prevents confusion when Claude would invoke a command that just redirects to a skill anyway.
...
Fixed a confusing pattern where Claude would invoke a skill via the Skill tool, then try to Read the skill file separately. The `using-harness` skill now explicitly states that the Skill tool loads skill content directly—no need to read files.

- Added "How to Access Skills" section to `using-harness`
- Changed "read the skill" → "invoke the skill" in instructions
- Updated slash commands to use fully qualified skill names (e.g., `harness:brainstorming`)
...
**Skill priority in using-harness**
...
- **Rationalizations table** in using-harness - Scannable format including new entries: "I need more context first", "Let me explore first", "This feels productive"
...
- **OpenCode Bootstrap Refactor**: Switched from `chat.message` hook to `session.created` event for bootstrap injection
  ...
  - Explicitly tells the model that using-harness is already loaded to prevent redundant skill loading
...
- **OpenCode Support**: Native JavaScript plugin for OpenCode.ai
  ...
  - Three-tier skill priority: project > personal > harness
...
- Optimized harness bootstrap to eliminate redundant skill execution. The `using-harness` skill content is now provided directly in session context, with clear guidance to use the Skill tool only for other skills. This reduces overhead and prevents the confusing loop where agents would execute `using-harness` manually despite already having the content from session start.
...
- Added unified `harness-codex` script with bootstrap/use-skill/find-skills commands
- Cross-platform Node.js implementation (works on Windows, macOS, Linux)
- Namespaced skills: `harness:skill-name` for harness skills, `skill-name` for personal
...
- Updated terminology: "Harness skills" instead of "Core skills"

### Files Added
- `.codex/INSTALL.md` - Installation guide for Codex users
- `.codex/harness-bootstrap.md` - Bootstrap instructions with Codex adaptations
- `.codex/harness-codex` - Unified Node.js executable with all functionality
...
**Updated using-harness skill to use Skill tool instead of Read tool**
...
### Files Changed
- Updated: `skills/using-harness/SKILL.md` - Changed tool references from Read to Skill

## v3.2.2 (2025-10-21)

### Improvements

**Strengthened using-harness skill against agent rationalization**
...
### Files Changed
- Updated: `skills/using-harness/SKILL.md` - Added three layers of enforcement to prevent skill-skipping rationalization
...
- Added `harness:code-reviewer` agent to plugin's `agents/` directory
- Agent provides systematic code review against plans and coding standards
- Previously required users to have personal agent configuration
- All skill references updated to use namespaced `harness:code-reviewer`
- Fixes #55

### Files Changed
- New: `agents/code-reviewer.md` - Agent definition with review checklist and output format
- Updated: `skills/requesting-code-review/SKILL.md` - References to `harness:code-reviewer`
- Updated: `skills/subagent-driven-development/SKILL.md` - References to `harness:code-reviewer`
...
- Updated format: `harness:test-driven-development` (previously just `test-driven-development`)
...
- **Fixed command syntax in README** (#44) - Updated all command references to use correct namespaced syntax (`/harness:brainstorm` instead of `/brainstorm`). Plugin-provided commands are automatically namespaced by Claude Code to avoid conflicts between plugins.
...
Harness v2.0 makes skills more accessible, maintainable, and community-driven through a major architectural shift.

The headline change is **skills repository separation**: all skills, scripts, and documentation have moved from the plugin into a dedicated repository ([astrosteveo/harness-skills](https://github.com/astrosteveo/harness-skills)). This transforms superpowers from a monolithic plugin into a lightweight shim that manages a local clone of the skills repository. Skills auto-update on session start. Users fork and contribute improvements via standard git workflows. The skills library versions independently from the plugin.
...
**The biggest change:** Skills no longer live in the plugin. They've been moved to a separate repository at [astrosteveo/harness-skills](https://github.com/astrosteveo/harness-skills).

**What this means for you:**

- **First install:** Plugin automatically clones skills to `~/.config/harness/skills/`
...
1. Your old `~/.config/harness/.git` will be backed up to `~/.config/harness/.git.bak`
2. Old skills will be backed up to `~/.config/harness/skills.bak`
3. Fresh clone of astrosteveo/harness-skills will be created at `~/.config/harness/skills/`
...
- Clones astrosteveo/harness-skills on first run
...
- `SUPERPOWERS_SKILLS_ROOT` set to `~/.config/harness/skills`
...
- Prominent link to harness-skills repo
...
**Removed:**
- `skills/` directory (82 files) - Now in astrosteveo/harness-skills
- `scripts/` directory - Now in astrosteveo/harness-skills/skills/using-skills/
- `hooks/setup-personal-superpowers.sh` - Obsolete

**Modified:**
- `hooks/session-start.sh` - Use skills from ~/.config/harness/skills
...
1. **Backup your personal skills** (if you have any):
   ```bash
   cp -r ~/.config/harness/skills ~/harness-skills-backup
   ```
...
- Skills repository is now at https://github.com/astrosteveo/harness-skills
...
**Full Changelog:** https://github.com/astrosteveo/harness/compare/dd013f6...main
**Skills Repository:** https://github.com/astrosteveo/harness-skills
